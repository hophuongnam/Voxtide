...
impl SpeakerMap {
    ...
    pub fn from_snapshot(snapshot: BTreeMap<String, char>) -> Self {
        let mut order: Vec<(String, char)> = snapshot.iter().map(|(k, v)| (k.clone(), *v)).collect();
    ...
    pub fn snapshot(&self) -> BTreeMap<String, char> { self.map.clone() }

    pub fn chip_for(&mut self, soniox_id: &str) -> char {
        if let Some(c) = self.map.get(soniox_id) { return *c; }
